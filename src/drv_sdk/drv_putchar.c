...
_attribute_ram_code_ void soft_uart_putc(unsigned char byte)
{
    ...
    u16 bit[10] = {0};
#else
    u8 tmp_bit0 = reg_gpio_out(device->debug_gpio.gpio) & (~(device->debug_gpio.gpio & 0xff));
    u8 tmp_bit1 = reg_gpio_out(device->debug_gpio.gpio) | (device->debug_gpio.gpio & 0xff);
    u8 bit[10] = {0};
#endif
    ...
    for (j = 0; j < 10; j++) {
        ...
        reg_gpio_out(device->debug_gpio.gpio) = bit[j]; //send bit0
    }
    //drv_restore_irq(r);
}
#endif
